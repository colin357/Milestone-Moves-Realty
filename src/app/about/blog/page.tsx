        ...
        {posts.map((post) => (
          <article key={post.slug} className="border border-gray-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex items-center gap-3 mb-3">
              <span className="bg-[#2BB8D3] text-white text-xs font-bold px-3 py-1 rounded-full">{post.tag}</span>
              <span className="text-gray-400 text-sm">{post.date}</span>
            </div>
            <h2 className="text-xl font-bold text-[#1C2B5E] mb-2">{post.title}</h2>
            <p className="text-[#3d3d3d] text-sm leading-relaxed mb-4" dangerouslySetInnerHTML={{ __html: post.excerpt }} />
            <Link href={`/about/blog/${post.slug}`} className="text-[#2BB8D3] font-bold text-sm hover:underline">
              Read More →
            </Link>
          </article>
        ))}
