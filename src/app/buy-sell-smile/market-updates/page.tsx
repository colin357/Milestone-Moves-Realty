...
export default function MarketUpdates() {
  return (
    <div>
      ...
      <section className="relative overflow-hidden bg-[#1C2B5E] py-24 px-4">
        <div className="absolute inset-0 opacity-20">
          <Image src="/images/market-updates.jpg" alt="" fill className="object-cover" sizes="100vw" />
        </div>
        <div className="absolute inset-0 bg-[#1C2B5E]/70" />
      ...
      <section className="bg-[#2BB8D3] py-6 px-4">
      ...
      <section className="py-20 px-4 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-[#2BB8D3] font-bold tracking-widest text-sm mb-3 uppercase">Why the Triangle</p>
            ...
            <div className="grid grid-cols-2 gap-4">
              {whyTriangle.map((item) => (
                <div key={item.label} className="bg-[#1C2B5E] text-white rounded-2xl p-6">
      ...
      <section className="py-20 px-4 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-[#2BB8D3] font-bold tracking-widest text-sm mb-3 uppercase">Area by Area</p>
            <h2 className="text-3xl font-bold text-[#1C2B5E]">Neighborhood Market Snapshots</h2>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {neighborhoods.map((n) => (
              <div key={n.area} className="bg-white rounded-2xl p-6 shadow-sm border-l-4 border-[#2BB8D3]">
                <div className="flex justify-between items-start mb-3">
                  <h3 className="font-bold text-[#1C2B5E] text-xl">{n.area}</h3>
                  <span className={`text-xs font-bold px-2 py-1 rounded-full ${
                    n.tag === "Seller's Market" ? "bg-[#2BB8D3] text-white" :
                    n.tag === "Rising" ? "bg-[#1C2B5E] text-white" :
                    "bg-gray-100 text-gray-600"
                  }`}>{n.tag}</span>
                </div>
                <p className="text-[#2BB8D3] font-bold text-lg mb-2">{n.trend}</p>
                <p className="text-[#3d3d3d] text-sm leading-relaxed">{n.detail}</p>
              </div>
            ))}
          </div>
          <p className="text-center text-gray-400 text-xs mt-6">Market conditions as of Q2 2025 and subject to change. Contact us for current data on your specific area.</p>
        </div>
      </section>
      ...
      <section className="py-20 px-4 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-[#2BB8D3] font-bold tracking-widest text-sm mb-3 uppercase">What This Means For You</p>
            <h2 className="text-3xl font-bold text-[#1C2B5E]">How to Navigate Today&apos;s Market</h2>
          </div>
          <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-[#f0fafd] border border-[#2BB8D3] rounded-2xl p-8">
              <h3 className="font-bold text-[#1C2B5E] text-xl mb-6 flex items-center gap-3">
                <span className="w-10 h-10 bg-[#2BB8D3] text-white rounded-full flex items-center justify-center text-lg">🔍</span>
                If You&apos;re Buying
              </h3>
              <ul className="space-y-3">
                {buyerTips.map((tip) => (
                  <li key={tip} className="flex gap-3 items-start text-sm text-[#3d3d3d]">
                    <span className="w-5 h-5 bg-[#2BB8D3] text-white rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 text-xs font-bold">✓</span>
                    {tip}
                  </li>
                ))}
              </ul>
            </div>
            <div className="bg-[#1C2B5E] rounded-2xl p-8">
              <h3 className="font-bold text-white text-xl mb-6 flex items-center gap-3">
                <span className="w-10 h-10 bg-[#2BB8D3] text-white rounded-full flex items-center justify-center text-lg">🏷️</span>
                If You&apos;re Selling
              </h3>
              <ul className="space-y-3">
                {sellerTips.map((tip) => (
      ...
      {/* Report CTA */}
      <section className="relative py-20 px-4 bg-[#1C2B5E] text-white text-center overflow-hidden">
