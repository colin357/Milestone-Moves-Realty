  ...
  return (
    <div>
      ...
      <section className="relative min-h-[60vh] flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-[#1C2B5E]/95 via-[#1C2B5E]/80 to-transparent" />
        <div className="relative z-10 max-w-6xl mx-auto px-6 py-24 w-full">
          ...
          <div className="flex flex-wrap gap-4">
            <Link href="/about/contact-us" className="bg-[#2BB8D3] hover:bg-[#1fa3bd] text-white font-bold px-8 py-4 rounded-full transition-colors text-sm tracking-wider">
              START YOUR SEARCH
            </Link>
            <Link href="/55-plus-options/simplified-living-communities" className="border-2 border-white hover:bg-white hover:text-[#1C2B5E] text-white font-bold px-8 py-4 rounded-full transition-colors text-sm tracking-wider">
      ...
      {/* Stats strip */}
      <section className="bg-[#2BB8D3] py-8 px-4">
      ...
      <section className="py-20 px-4 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-[#2BB8D3] font-bold tracking-widest text-sm mb-3 uppercase">Our Buyer Process</p>
            <h2 className="text-3xl sm:text-4xl font-bold text-[#1C2B5E]">Eight Steps. Zero Surprises.</h2>
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {steps.map((step) => (
              <div key={step.n} className="flex gap-5 items-start bg-gray-50 rounded-2xl p-6 hover:bg-[#f0fafd] transition-colors group">
                <span className="text-4xl font-black text-[#2BB8D3]/20 group-hover:text-[#2BB8D3]/40 transition-colors leading-none flex-shrink-0 w-12">
                  {step.n}
                </span>
                <div>
                  <h3 className="font-bold text-[#1C2B5E] text-lg mb-1">{step.title}</h3>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={c.item} className={`flex gap-3 items-center rounded-xl p-4 ${c.ready ? "bg-white/10" : "bg-white/5 border border-white/10"}`}>
                <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${c.ready ? "bg-[#2BB8D3]" : "border-2 border-white/30"}`}>
                  {c.ready && (
      ...
      <section className="py-20 px-4 bg-gray-50">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            <p className="text-[#2BB8D3] font-bold tracking-widest text-sm mb-3 uppercase">Common Questions</p>
            <h2 className="text-3xl font-bold text-[#1C2B5E]">Buyer FAQ</h2>
          </div>
          <div className="space-y-4">
            {faqs.map((faq) => (
              <div key={faq.q} className="bg-white rounded-2xl overflow-hidden shadow-sm">
                <div className="flex gap-4 p-6">
                  <div className="w-8 h-8 bg-[#2BB8D3] text-white rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 mt-0.5">Q</div>
                  <div>
                    <p className="font-bold text-[#1C2B5E] mb-2">{faq.q}</p>
